...
public class BinaryHeap<T extends Comparable<? super T>> {
    ...
    void heapify(int n, int i) {
        ...
        if(leftChild < n && pq[leftChild].compareTo(pq[smallest]) == -1) {
            smallest = leftChild;
        }
        else if (rightChild < n && pq[rightChild].compareTo(pq[smallest]) == -1) {
            smallest = rightChild;
        }
    ...
    void percolateUp(int i) { /* to be implemented */
        T data = pq[i];
        while (i > 0 && data.compareTo(pq[parent(i)]) == -1) {
            pq[i] = pq[parent(i)];
            i  = parent(i);
        }
        pq[i] = data;
    ...
    void percolateDown(int i) { /* to be implemented */
        ...
        while(child < size) {
            if (child + 1 < size  && pq[child].compareTo(pq[child+1]) == 1) {
                child = child + 1;
            }
            if (pq[child].compareTo(data) == 1) {
                break;
            }
            pq[i] = pq[child];
            i  = child;
...
// end of optional problem: kth largest element (Q3)
    ...
    public static void main(String args[]) {
        ...
        arr[3] = 2;
        arr[4] = 9;
        BinaryHeap<Integer> q = new BinaryHeap<>(arr, ((Integer a, Integer b) -> a.compareTo(b)), 5);
        q.printHeap();
